...
class MultiplicativeInteractions3Modal(nn.Module):
    ...
    def __init__(self, input_dims, output_dim, task=None):
        ...
        # reduce the dimension of the input modalities to avoid OOM
        dim_hidden = 64
        self.fc = nn.ModuleList([nn.Linear(input_dim, dim_hidden) for input_dim in input_dims])
        #
        self.a = MultiplicativeInteractions2Modal([dim_hidden, dim_hidden], [dim_hidden, output_dim], "matrix3D")
        self.b = MultiplicativeInteractions2Modal([dim_hidden, dim_hidden], output_dim, "matrix")
        self.task = task
    ...
    def forward(self, modalities):
        ...
        """
        # reduce the dimension of the input modalities to avoid OOM
        modalities = [F.relu(fc(modality)) for fc, modality in zip(self.fc, modalities)]
        if self.task == "affect":
            return torch.einsum("bm, bmp -> bp", modalities[2], self.a(modalities[0:2])) + self.b(modalities[0:2])
        return (torch.matmul(modalities[2], self.a(modalities[0:2])) + self.b(modalities[0:2])).view(-1, self.output_dim)
...
class TensorFusion(nn.Module):
    ...
    def __init__(self, input_dims, inter_dim=32, output_dim=128):
        """Instantiates TensorFusion Network Module."""
        super().__init__()
        pre_fc = []
        for input_dim in input_dims:
            pre_fc.append(nn.Sequential(nn.Linear(input_dim, inter_dim), nn.ReLU()))
        self.pre_fc = nn.ModuleList(pre_fc)
        self.post_fc = nn.Sequential(nn.Linear((inter_dim + 1) ** (len(input_dims)), output_dim))
    ...
    def forward(self, modalities):
        ...
        for idx in range(len(modalities)):
            modalities[idx] = self.pre_fc[idx](modalities[idx])
...
class LowRankTensorFusion(nn.Module):
    ...
    def forward(self, modalities):
        ...
        for modality, factor in zip(modalities, self.factors):
            ones = Variable(torch.ones(batch_size, 1).type(modality.dtype), requires_grad=False).to("cuda" if torch.cuda.is_available() else "cpu")
            if self.flatten:
                modality_withones = torch.cat((ones, torch.flatten(modality, start_dim=1)), dim=1)
            else:
                modality_withones = torch.cat((ones, modality), dim=1)
            modality_factor = torch.matmul(modality_withones, factor.to("cuda" if torch.cuda.is_available() else "cpu"))
