...
def define_loss(args):
    ...
    elif args.loss == "nll_surv_kd_kd":
        loss = [NLLSurvLoss(alpha=0.0), DistillKL(), DistillKL()]
    elif args.loss == "nll_surv_kd_hint":
        loss = [NLLSurvLoss(alpha=0.0), DistillKL(), HintLoss()]
    else:
...
def nll_loss(hazards, S, Y, c, alpha=0.4, eps=1e-7):
    ...
    # hazards = hazards.half()  # Cast hazards tensor to float16 (half-precision)
    if S is None:
